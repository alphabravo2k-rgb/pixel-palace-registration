...
const ERROR_CODES = {
  ...
  TOURNAMENT_FULL: { code: "ERR011", message: "Tournament slots are fully filled." },
  REGISTRATION_CLOSED: { code: "ERR012", message: "Registration is closed." }
};
...
function doGet(e) {
  try {
    ...
    if (endpoint === "/api/v1/getSlots") {
      const slots = TournamentLifecycleService.calculate(params.tournamentId, config);
...
const DatabaseAdapter = {
  getTournamentConfig: function (tournamentId, bypassCache) {
...
const RegistrationService = {
  registerTeam: function (payload) {
    ...
    // 1. Refresh Config (bypass cache)
    const config = DatabaseAdapter.getTournamentConfig(tournamentId, true);
    if (!config || !config.tournamentid) {
      return { success: false, error: ERROR_CODES.TOURNAMENT_NOT_FOUND.message, errorCode: ERROR_CODES.TOURNAMENT_NOT_FOUND.code };
    }

    // 2. Validate Submission State (reject if closed, full, archived, etc.)
    try {
      TournamentLifecycleService.validateSubmission(tournamentId, config);
    } catch (lifecycleErr) {
      const state = TournamentLifecycleService.calculate(tournamentId, config);
      const reason = state.registration.closedReason;
      let errObj = ERROR_CODES.REGISTRATION_CLOSED;
      if (reason === "SLOT_LIMIT_REACHED") {
        errObj = ERROR_CODES.TOURNAMENT_FULL;
      }
      return { success: false, error: errObj.message, errorCode: errObj.code, reason: reason };
    }

    // 3. PreConditions Check (Payload format)
    ...
    // 4. Business Rules Check (Duplicates, invite codes, bans)
    const businessRes = this.validateBusinessRules_(payload, config);
    if (!businessRes.valid) {
      let errObj = ERROR_CODES.DUPLICATE_TEAM;
      if (businessRes.error === "INVALID_INVITE") {
        errObj = ERROR_CODES.INVALID_INVITE;
      }
      return { success: false, error: errObj.message, errorCode: errObj.code, reason: businessRes.error };
    }
    ...
    AuditService.recordEvent(
      ...
      "NONE",
      "SUBMITTED",
      "Roster submitted for " + payload.team_name + " (Success: true)",
      "System"
    );

    // Refresh Registration Data & Recalculate Lifecycle (triggers FSM audit logging internally)
    TournamentLifecycleService.calculate(tournamentId, config);
...
const TournamentLifecycleService = {
  ...
  calculate: function(tournamentId, config) {
    ...
    return {
      max: maxTeams,
      confirmed: activeTeams.length,
      left: remainingSlots,
      inviteConfirmed: inviteUsed,
      openConfirmed: publicUsed,
      isFull: isFull,
      phase: phase,
      registration: {
        status: status,
      ...
      meta: {
        ...
        generationTimeMs: 10,
        cacheStatus: "MISS",
        requestId: "REQ-" + Utilities.getUuid().substring(0, 8).toUpperCase(),
