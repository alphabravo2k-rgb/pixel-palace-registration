...
const WEB_APP_URL = "https://script.google.com/macros/s/AKfycbxh_jxh7bgnrsAJHrlNW0UNZ7yrKnmW5qgYnyGiGMAUWnbPXUj9yZVA8OhTOEB-l8fV/exec"
...
function updateFaceitData(force) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    try {
      ...
      var badgeVal = getCS2RankBadge(skillLvl, row);
      if (badgeVal.indexOf("=") === 0) {
        sheet.getRange(row, C.RANK_BADGE).setFormula(badgeVal);
      } else {
        sheet.getRange(row, C.RANK_BADGE).setValue(badgeVal);
      }
...
function parseRole(name) {
  ...
  if (nameStr.indexOf("(C)") !== -1) return "Captain";
  if (nameStr.indexOf("(Partner)") !== -1) return "Partner";
  if (nameStr.indexOf("(Sub)") !== -1) return "Substitute";
  return "X";
}
...
/**
 * Optional Web App GET entry point.
 * Allows deploying the Admin Ops script directly as a Web App to serve dynamic badge SVGs.
 */
function doGet(e) {
  try {
    ...
    // Route request to badge generator if badge/level/elo query params are present
    if (endpoint === "/api/v1/badge" || params.action === "badge" || params.level || params.elo) {
      var level = params.level;
      var elo = params.elo;
      var color = "#FFFFFF";

      if (elo !== undefined && elo !== null && elo !== "") {
        var eloVal = parseInt(elo, 10) || 0;
        var rankData = getLevelData_(eloVal);
        level = rankData.level;
        color = rankData.color;
      } else if (level !== undefined && level !== null && level !== "") {
        if (level === "Challenger" || level.toString().toLowerCase() === "challenger") {
          level = "Challenger";
          color = "#FF1E27";
        } else {
          var numLevel = parseInt(level, 10) || 1;
          var match = SEEDS_.find(function (s) { return s.level === numLevel; });
          level = numLevel;
          if (match) {
            color = match.color;
          }
      ...
      var svg = getFaceitBadgeSVG_(level, color);
...
// Private helpers for badge rendering (ES5 compatible)
var SEEDS_ = [
  { max: 500, level: 1, color: "#FFFFFF" },
  { max: 750, level: 2, color: "#00E65A" },
  { max: 900, level: 3, color: "#00E65A" },
  { max: 1050, level: 4, color: "#FFC800" },
  { max: 1200, level: 5, color: "#FFC800" },
  { max: 1350, level: 6, color: "#FFC800" },
  { max: 1530, level: 7, color: "#FFC800" },
  { max: 1750, level: 8, color: "#FF5E00" },
  { max: 2000, level: 9, color: "#FF5E00" },
  { max: 2500, level: 10, color: "#FF1E27" },
  { max: 9999999, level: 'Challenger', color: "#FF1E27" }
];

function getLevelData_(elo) {
  for (var i = 0; i < SEEDS_.length; i++) {
    if (elo <= SEEDS_[i].max) return SEEDS_[i];
  }
  return SEEDS_[SEEDS_.length - 1];
}

function getFaceitBadgeSVG_(level, color) {
  if (level === 'Challenger') {
    return '<svg width="40" height="40" viewBox="0 0 34 34" xmlns="http://www.w3.org/2000/svg">\n' +
      '  <circle cx="17" cy="17" r="15" fill="#111111"/>\n' +
      '  <circle cx="17" cy="17" r="15" fill="none" stroke="' + color + '" stroke-width="4" stroke-dasharray="3 3.5"/>\n' +
      '  <polygon points="12,10 12,24 23,17" fill="' + color + '" />\n' +
      '</svg>';
  }

  var circumference = 2 * Math.PI * 15; // ~94.24
  var dashLength;
  ...
  if (level === 10) {
    ...
    dashLength = (level / 10) * circumference;
  }

  return '<svg width="40" height="40" viewBox="0 0 34 34" xmlns="http://www.w3.org/2000/svg">\n' +
    '  <circle cx="17" cy="17" r="15" fill="#111111"/>\n' +
    '  <circle cx="17" cy="17" r="15" fill="none" stroke="' + color + '" stroke-width="4" stroke-linecap="round" stroke-dasharray="' + dashLength + ' ' + circumference + '" transform="rotate(130 17 17)" />\n' +
    '  <text x="17" y="21.5" text-anchor="middle" fill="' + color + '" font-size="13" font-weight="800" font-family="Arial, sans-serif">' + level + '</text>\n' +
    '</svg>';
}
