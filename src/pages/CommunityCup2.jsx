...
import { LiveTracker } from './LiveTracker'; // Ensure this matches where you saved the tracker from my last message
...
export default function CommunityCup2() {
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      ...
      } catch (e) { setSlots(s => ({ ...s, loading: false })); }
    };
    fetchSlots();
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-7xl mx-auto">
      ...
      <div className="lg:col-span-8">
        <div className="flex justify-center gap-12 w-full mt-2 mb-8 relative">
            ...
            <button onClick={() => setActiveTab('register')} className={`cyber-tab ${activeTab === 'register' ? 'active' : ''}`}>Register Team</button>
            <button onClick={() => setActiveTab('tracker')} className={`cyber-tab flex items-center gap-3 ${activeTab === 'tracker' ? 'active' : ''}`}>Live Roster Tracker <span className="bg-[var(--neon-pink)] text-white text-[10px] px-2 py-0.5 rounded-sm font-sans font-bold tracking-widest animate-pulse">LIVE</span></button>
        </div>

        {activeTab === 'register' ? (
            <RegistrationForm 
              API_URL={TOURNAMENT_API_URL} 
              tournamentId="community-cup-2" 
